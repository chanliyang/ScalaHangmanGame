...
class GameController2(
  ...
  private val word2 = "dirty"
  private val levelnumber = 2

  private var guessedWord = Array.fill(word2.length)('*')
  ...
  def submit(): Unit = {
    if (charTextField.text.value.nonEmpty && remainingGuesses > 0) {
      ...
      if (guessedChars.contains(enteredChar)) {
        ...
        if (word2.contains(enteredChar)) {
          wordLabel.text = s"'$enteredChar' is in the word!"
          for (i <- word2.indices) {
            if (word2(i) == enteredChar) guessedWord(i) = enteredChar
          }
          wordGuessing.text = guessedWord.mkString
        } else {
          remainingGuesses -= 1
        ...
        if (guessedWord.mkString == word2) {
          wordLabel.text = "Yay! The girl is saved!"
          disableInput()
  ...
  def retry(): Unit = {
    guessedWord = Array.fill(word2.length)('*')
    guessedChars = ""
    remainingGuesses = 5
    wordLabel.text = "Guess the word:"
    wordGuessing.text = guessedWord.mkString
