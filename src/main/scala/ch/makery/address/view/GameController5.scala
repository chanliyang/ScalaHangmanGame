...
class GameController5(
  ...
  private val word5 = "scary"
  private val levelnumber = 5

  private var guessedWord = Array.fill(word5.length)('*')
  ...
  def submit(): Unit = {
    if (charTextField.text.value.nonEmpty && remainingGuesses > 0) {
      ...
      if (guessedChars.contains(enteredChar)) {
        ...
        if (word5.contains(enteredChar)) {
          wordLabel.text = s"'$enteredChar' is in the word!"
          for (i <- word5.indices) {
            if (word5(i) == enteredChar) guessedWord(i) = enteredChar
          }
          wordGuessing.text = guessedWord.mkString
        } else {
          remainingGuesses -= 1
        ...
        if (guessedWord.mkString == word5) {
          wordLabel.text = "The ghost didn't kill the boy"
          disableInput()
  ...
  def retry(): Unit = {
    guessedWord = Array.fill(word5.length)('*')
    guessedChars = ""
    remainingGuesses = 5
    wordLabel.text = "Guess the word:"
    wordGuessing.text = guessedWord.mkString
